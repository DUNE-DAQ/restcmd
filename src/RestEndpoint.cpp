...
void
RestEndpoint::init(size_t threads)
{
...
inline void
extendHeader(Http::Header::Collection& headers)
{
...
void
RestEndpoint::handle_route_command(const Rest::Request& request, Http::ResponseWriter response)
{
...
void
RestEndpoint::handleResponseCommand(const cmdobj_t& cmd, dunedaq::cmdlib::cmd::CommandReply& meta)
{
  ...
  response.then([&](Http::Response response) { TLOG() << "Response code = " << response.code(); },
                [&](std::exception_ptr exc) {
                  // handle response failure
                  try {
                    std::rethrow_exception(exc);
                  } catch (const std::exception& e) {
                    TLOG() << "Exception thrown by Http::Client::post() call: \"" << e.what()
                           << "\"; errno = " << errno;
                  }
                });
